...
async.series(
  [
    ...
    function getLatestRow(step) {
      //console.log(sheet);
      sheet.getRows({ offset: sheet.rowCount - 1, limit: 1 }, function(
        err,
        rows
      ) {
        latestRowDate = new moment(rows[0].date);
        console.log(
          "Step 3: Get Latest Row Data Successful, " +
            latestRowDate.format("YYYY-MM-DD")
        );
        step();
      });
    },
    ...
    function getContract(step) {
      ...
      function findContract() {
        scraperjs.StaticScraper.create(
          ...
          .scrape(function($) {
            return $(
              "#calendarFuturesProductTable1 > tbody > tr:nth-child(" + i + ")"
            )
              .map(function() {
                return $(this).text();
              })
          ...
          .then(function(text) {
            ...
            text1Array = text1Array.filter(function(n) {
              return n != "" && n != "--";
            });
    ...
    function addMissingDataToSheet(step) {
      ...
      async.series(functions, function(err) {
        ...
        console.log("Step 5: Push Missing Data To Sheet Successful");
        step();
      });
    },
    ...
    function getUpdatedSheet(step) {
      updatedDoc.getInfo(function(err, info) {
        ...
        console.log(
          "Step 7: Get Updated Sheet Successful, " +
    ...
    function getAllRows(step) {
      updatedSheet.getRows(
        { offset: 1, limit: updatedSheet.rowCount },
        function(err, rows) {
          // format data
          jsonData = clone(rows);
          ...
          console.log(
            "Step 8: Get All Rows Successfull, " +
    ...
    function convertDataToCsv(step) {
      json2csv({ data: jsonData, fields: Object.keys(jsonData[0]) }, function(
        err,
        csv
      ) {
        if (err) {
    ...
    function uploadCsvToS3(step) {
      ...
      s3.putObject(params, function(err, data) {
        if (err) {
          ...
          console.log(
            "Step 10: Successfully Uploaded Data To " +
              bucketName +
              "/" +
              keyName
          );
    ...
    function createMlTrainingDatasource(step) {
      ...
      var params = {
        ...
        DataSourceName:
          "Princess Strategy Training 0-70 " + moment().format("YYYY-MM-DD")
      };
    ...
    function createMlEvaluationDatasource(step) {
      ...
      var params = {
        ...
        ComputeStatistics: true,
        DataSourceName:
          "Princess Strategy Evaluation 70-100 " + moment().format("YYYY-MM-DD")
      };
        ...
        mL.getMLModel(params, function(err, data) {
          if (err) {
            ...
            if (data.Status !== "COMPLETED") {
              console.log(
                "Step 14: Machine Learning Model is " +
                  data.Status +
                  ", waiting 30 sec..."
              );
              sleep.sleep(30);
              ...
              mL.createRealtimeEndpoint(params, function(err, data) {
                if (err) {
                  ...
                  console.log(
                    "Step 14: Realtime Prediction Endpoint Created Successfully"
                  );
                  step();
                }
              });
            ...
            if (data.EndpointInfo.EndpointStatus !== "READY") {
              ...
              mL.predict(params, function(err, data) {
                if (err) {
                  ...
                  console.log(
                    "Step 15: Predict Binary Change Direction Successful, " +
                      predictionDirection
                  );
                  step();
                }
        ...
        [
          ...
          function createCampaign(mS) {
            var body = {
              ...
              settings: {
                subject_line:
                  "Dutchess.ai QM/CL Prediction for " +
                  predDate.format("MM/DD/YYYY"),
                preview_text:
                  "Dutchess.ai predicts " +
                  contract +
                  " will move " +
                  predictionDirection +
                  " from $" +
...
` +
              contract +
              ` price change direction prediction for ` +
              predDate.format("MM/DD/YYYY") +
              `: 
` +
              predictionDirection +
...
Order suggestion based on prediction + cross reference of mid pivot point for ` +
              predDate.format("MM/DD/YYYY") +
              `:
` +
              predictionPosition +
