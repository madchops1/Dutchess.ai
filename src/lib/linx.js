/*
 ...
 ( o.o )
  > ^ <
DUTCHESS.AI - "LINX"
A progressively smarter machine learning litecoin momentum prediction module

NOTES:
Not used... currently just progressed nanana.js
...
const sheetId = secrets.LinxSheetId;
  ...
  [
    ...
    function getInfoAndWorksheets(step) {
      doc.getInfo(function(err, info) {
        ...
        console.log(
          "Step 2: Get Sheet Successful, " + sheet.rowCount + " rows"
        );
        step();
      });
    },
    ...
    function clearSheet(step) {
      sheet.getRows({}, function(err, rows) {
        ...
        async.series(functions, function(err) {
          ...
          console.log("Step X: Clear Sheet Successful");
          step();
        });
      });
    },
    ...
    function getTrendData(step) {
      ...
      googleTrends
        ...
        .then(function(results) {
          ...
          console.log(
            "Step 4: Get Historical Google Trends Data Successful,",
            missingData.length
          );
          step();
        })
    ...
    function getPriceDataCoinbase(step) {
      ...
      for (var k in missingData) {
        var fun = function(ss) {
          ...
          coinbase.getSpotPrice(
            { currencyPair: "BTC-USD", date: date },
            function(err, obj) {
              //console.log(dots+'\r');
      ...
      async.series(functions, function(err) {
        ...
        console.log("Step 5.1: Get Price Data From Coinbase Successful");
        step();
      });
    },
    ...
    function getPriceDataGdax(step) {
      ...
      publicClient.getProductHistoricRates(
        "BTC-USD",
        { start: start, end: end, granularity: 86400 },
        function(err, res) {
          if (err) {
            console.log("ERR", err);
          }
          ...
          for (var k in data) {
            for (var l in missingData) {
              if (
                moment.unix(data[k][0]).format("YYYY-MM-DD") ==
                moment.unix(missingData[l].time).format("YYYY-MM-DD")
              ) {
                //console.log(dots+'\r');
                //process.stdout.write(dots+'\r');
                //dots += '.';

                missingData[l].date = moment
                  .unix(missingData[l].time)
                  .format("YYYY-MM-DD");
    ...
    function addDataToSheet(step) {
      ...
      while (k < data.length) {
        ...
        var fun = function(ss) {
          ...
          if (newRows[k].date) {
            sheet.addRow(newRows[k], function(err) {
              if (err) {
                console.log(err);
              }
      ...
      async.series(functions, function(err) {
        ...
        console.log("Step 6: Push Data To Sheet Successful");
        step();
      });
    },
    ...
    function convertCsv(step) {
      json2csv({ data: newRows, fields: Object.keys(newRows[0]) }, function(
        err,
        csv
      ) {
        if (err) {
    ...
    function uploadCsvToS3(step) {
      ...
      s3.putObject(params, function(err, data) {
        if (err) {
          ...
          console.log(
            "Step 8: Successfully Uploaded Data To " +
              bucketName +
              "/" +
              keyName
          );
    ...
    function createMlTrainingDatasource(step) {
      ...
      var params = {
        ...
        DataSpec: {
          ...
          DataRearrangement:
            '{"splitting":{"percentBegin":0, "percentEnd":70, "strategy":"random"}}',
          DataSchema: dataSchemaWinLoss
        },
        ComputeStatistics: true,
        DataSourceName:
          "Tiger Strategy Training 0-70 " + moment().format("YYYY-MM-DD")
      };
    ...
    function createMlEvaluationDatasource(step) {
      ...
      var params = {
        ...
        ComputeStatistics: true,
        DataSourceName:
          "Tiger Strategy Evaluation 70-100 " + moment().format("YYYY-MM-DD")
      };
      mL.createDataSourceFromS3(params, function(err, data) {
        if (err) {
          console.log(err, err.stack);
        ...
        mL.getMLModel(params, function(err, data) {
          if (err) {
            ...
            if (data.Status !== "COMPLETED") {
              console.log(
                "Step 12: Machine Learning Model is " +
                  data.Status +
                  ", waiting 30 sec..."
              );
              sleep.sleep(30);
              ...
              mL.createRealtimeEndpoint(params, function(err, data) {
                if (err) {
                  ...
                  console.log(
                    "Step 12: Realtime Prediction Endpoint Created Successfully"
                  );
                  step();
                }
              });
    ...
    function getCurrentPrice(step) {
      ...
      coinbase.getSpotPrice({ currencyPair: "BTC-USD", date: date }, function(
        err,
        obj
      ) {
        if (err) {
          ...
          console.log(
            "Step 13: Get Current Bitcoin Price Successful",
            currentPrice
          );
          step();
        }
        ...
        mL.getMLModel(params, function(err, data) {
          if (err) {
            ...
            if (data.EndpointInfo.EndpointStatus !== "READY") {
              console.log(
                "Step 14: Endpoint is " +
                  data.EndpointInfo.EndpointStatus +
                  ", waiting 30 sec..."
              );
              sleep.sleep(30);
              ...
              mL.predict(params, function(err, data) {
                if (err) {
                  ...
                  predictionDirection =
                    prediction == 1 ? "Upwards" : "Downwards";
        ...
        [
          ...
          function sendSms(mS) {
            sms
              .send(
                "+16302175813",
                "Dutchess.ai predicts Bitcoin will move " +
                  predictionDirection +
                  " from $" +
                  currentPrice +
                  " for " +
                  predDate.format("MM/DD/YYYY"),
                test
              )
              .then(function() {
                mS();
          ...
          function createCampaign(mS) {
            var body = {
              ...
              settings: {
                subject_line:
                  "Dutchess.ai Bitcoin Prediction for " +
                  predDate.format("MM/DD/YYYY"),
                preview_text:
                  "Dutchess.ai predicts Bitcoin will move " +
                  predictionDirection +
...
Bitcoin (GDAX) price change direction prediction for ` +
              predDate.format("MM/DD/YYYY") +
              `: 
` +
              predictionDirection +
...
Current Bitcoin (GDAX) Price for ` +
...
Latest available historical quote for ` +
              latestRowDate.format("MM/DD/YYYY") +
              `:
Open: ` +
              openPrice +
              `  
...
or ** unsubscribe from this list (*|UNSUB|*)`;
            ...
            mailchimp
              .put({
                path: "/campaigns/" + campaign.id + "/content",
                body: body
              })
              .then(function(result) {
          ...
          function sendTestEmail(mS) {
            ...
            mailchimp
              .post({
                path: "/campaigns/" + campaign.id + "/actions/test",
                body: body
              })
              .then(function(result) {
          ...
          function sendLiveEmail(mS) {
            if (!test) {
              mailchimp
                .post({ path: "/campaigns/" + campaign.id + "/actions/send" })
                .then(function(result) {
