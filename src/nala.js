    ...
    [
        ...
        function configMachieLearning(step) {
            if (machineLearning === true) {
                returnJsonFileObject(constants.TMP + '/nala/endpoint.json').then(function(data, err) {
...
function handleTrading(data) {
    return new Promise(function(resolve, reject) {
        console.log('Status', volumeCrossesUpwards(), momentumIsUp(), momentumIsDown());
        ...
        if (volumeCrossesUpwards() && momentumIsUp() && sellSignal) {
            ...
            if (holdingData) {
                console.log('HOLDING', profit, profitTarget, stopLoss, totalProfit - totalFees, winners, losers);
...
function buy(data) {
    ...
    return new Promise(function(resolve, reject) {
        price = data.price;
        fix.placeOrder('buy', 'limit', tradeAmountCoin, coin[0], false, price, 'FOK').then(function(data, err) {
            if (err) {
                console.log(err);
                reject(err);
            }
...
function sell(data, status) {
    ...
    return new Promise(function(resolve, reject) {
        price = data.price;
        fix.placeOrder('sell', 'limit', tradeAmountCoin, coin[0], false, price, 'FOK').then(function(data, err) {
            if (err) {
                console.log(err);
                reject(err);
            }
...
function calculateVolume() {
    ...
    let shortVolumeAvg = getArrayAvg([volumeData[volumeData.length - 1], volumeData[volumeData.length - 2]]);
