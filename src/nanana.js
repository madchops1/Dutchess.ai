    ...
    if (tickerData.length > ticks - 1) {
        ...
        if (currentState > overallAvg && currentAvgs[currentAvgs.length - 2] <= overallAvgs[overallAvgs.length - 2]) {
            ...
            if (thirdAvg > secondAvg) {
                ...
                if (!holdingData) {
                    if (realTimeEndpoint) {
                        let lastSvl = currentAvgs[currentAvgs.length - 2] - overallAvgs[overallAvgs.length - 2];
                        let currentSvl = currentState - overallAvg;
                        let rocAlpha = parseFloat(parseFloat(thirdAvg) / parseFloat(secondAvg) * 100).toFixed(3);
                        let rocBeta = parseFloat(parseFloat(secondAvg) / parseFloat(firstAvg) * 100).toFixed(3);
              ...
              */
                            ...
                            mL.predict(params, function(err, prediction) {
                                if (err) {
                                    ...
                                    if (prediction.Prediction.predictedLabel == '1') {
                                        ++orderCount;
                                        ...
                                        if (!test) {
                                            fix
                                                .placeOrder('buy', 'market', tradeAmountCoin, coin[0], false)
            ...
            if (holdingData) {
                if (profit >= profitTarget) {
                    ...
                    if (!test) {
                        fix.placeOrder('sell', 'market', tradeAmountCoin, coin[0], false).then(function(dataa, err) {
                            if (err) {
                                console.log(err);
                            }
