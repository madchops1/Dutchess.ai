...
function calculateProfitLoss(data) {
    ...
    if (buyData && !sellData) {
        ...
        // reset profit
        profit = 0;
        profitTarget = tradeAmountCoin * data.price * target;
        stopLoss = tradeAmountCoin * data.price * risk * -1;
    }
    console.log(
...
function goodTimeToBuy() {
    ...
    console.log(moment().format('YYYY/MM/DD HH:mm:ss'), 'goodTimeToBuy()', false);
    return false;
}
...
function goodTimeToSell() {
    ...
    console.log(moment().format('YYYY/MM/DD HH:mm:ss'), 'goodTimeToSell()', false);
    return false;
}
...
function buy(prices) {
    return new Promise(function(resolve, reject) {
        ...
        //let price = parseFloat(parseFloat(book.asks[0][0])).toFixed(3);
...
function trader(data) {
    return new Promise(function(resolve, reject) {
        ...
        } else if (profit >= profitTarget && goodTimeToSell() && buyData.orderStatus == 'done' && !sellData) {
            // sell for profit
            sell(data, 1).then(function(data) {
                resolve();
            });
        } else if (profit < stopLoss && goodTimeToSell() && buyData.orderStatus == 'done' && !sellData) {
            // sell at a loss
